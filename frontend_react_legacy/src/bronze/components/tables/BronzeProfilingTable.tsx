...
interface BronzeProfilingTableProps {
  filteredColumns: ColumnProfile[];
}

export const BronzeProfilingTable: React.FC<BronzeProfilingTableProps> = ({ filteredColumns }) => {
  return (
      ...
      <table className="medallion-table" style={{ margin: 0 }}>
        <thead style={{ position: 'sticky', top: 0, zIndex: 10 }}>
          <tr>
            ...
            <th>Tipo Dato Crudo</th>
            <th>Nulos (% Incompletos)</th>
            <th>Únicos (% Varianza)</th>
          ...
          {filteredColumns.map((col) => {
            ...
            return (
              <tr key={col.column_name} style={{ backgroundColor: isConstant ? 'rgba(245, 158, 11, 0.04)' : undefined }}>
                ...
                <td>
                  <span
                    style={{
                      fontWeight: 700,
                      fontSize: '0.82rem',
                      color: col.null_count > 0 ? 'var(--accent-rose)' : 'var(--accent-emerald)',
                    }}
                  >
                ...
                <td style={{ fontSize: '0.78rem', color: 'var(--text-muted)', maxWidth: '300px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
